...
const AddWorkPage = ({ route, navigation }) => {
  ...
  const handleConfirm = async () => {
    ...
    const dropoff = await getCoordinates(workAddress);

    if (dropoff) {
      setConfirmedWorkAddress(dropoff);
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.inputContainer}>
        ...
        <View style={[styles.inputWrapper, { marginTop: 5 }]}>
          <TextInput
            ...
            value={workAddress}
            onChangeText={(text) => {
              setWorkAddress(text); // Use the correct setter function
              fetchSuggestions(text, setSuggestions);
            }}
          />
          {workAddress !== '' && (
            <TouchableOpacity onPress={() => setWorkAddress('')} style={styles.clearButton}>
        ...
        {confirmedWorkAddress && (
          <View style={styles.confirmedLocations}>
            <Text style={styles.locationText}>Work Location: {confirmedWorkAddress.name}</Text>
          </View>
        )}
      ...
      <TouchableOpacity
        onPress={confirmedWorkAddress ? handleSave : handleConfirm}
        style={styles.confirmButton}
      >
        <Text style={styles.confirmButtonText}>
          {confirmedWorkAddress ? 'Save' : 'Confirm'}
        </Text>
      </TouchableOpacity>
    </SafeAreaView>
  );
